	...
	render() {
		...
		return (
			<div className={classes.backdrop}>
				<Link onMouseOver={this.onHover} onMouseLeave={this.onUnHover} to={`/product/${id}`} className={`${mainClasses.container} ${mainClasses.column} ${classes.root} ${this.state.hovered ? classes.modal : ''}`}>
					<div className={`${mainClasses.container} ${mainClasses.column} ${classes.imgroot}`}>
						<div className={`${mainClasses.container} ${mainClasses.column} ${mainClasses.center}`}>
							<img className={classes.img} alt="product" src={gallery} />
						</div>
						{inStock ? (
							''
						) : (
					...
					<div className={`${mainClasses.container} ${mainClasses.row} ${mainClasses.space_around}`}>
						...
						<div className = {`${mainClasses.col_3} ${classes.cart}`}>
							{this.state.hovered ? (
								<button onClick = {this.handleAddToCart} style = {{backgroundColor : 'transparent' , border: 'none' , cursor : 'pointer'}}>
